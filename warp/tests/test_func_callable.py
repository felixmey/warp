...
def test_callable_builtin_func_parameter(test, device):
    """Verify Callable parameters accept simple built-in Warp functions."""
...
CALLABLE_BUILTIN_TARGET = wp.sin


@wp.kernel
def callable_builtin_global_target_kernel(out: wp.array[float]):
    out[0] = callable_apply_typing(CALLABLE_BUILTIN_TARGET, 0.5)
...
@wp.kernel
def callable_builtin_default_target_kernel(out: wp.array[float]):
    out[0] = callable_apply_builtin_default()


CALLABLE_OVERLOAD_TARGET = callable_double_it


@wp.func
def callable_apply_overloaded(x: int):
    return float(x)


@wp.func
def callable_apply_overloaded(g: TypingCallable, x: float):
    return g(x)


@wp.kernel
def callable_overloaded_global_target_kernel(out: wp.array[float]):
    out[0] = callable_apply_overloaded(CALLABLE_OVERLOAD_TARGET, 3.0)


@wp.func
def callable_apply_overloaded_default(x: int):
    return float(x)


@wp.func
def callable_apply_overloaded_default(x: float, g: TypingCallable = callable_double_it):
    return g(x)


@wp.kernel
def callable_overloaded_default_target_kernel(out: wp.array[float]):
    out[0] = callable_apply_overloaded_default(3.0)
...
CALLABLE_DEPENDENCY_OVERLOAD_PROVIDER_MODULE = wp.Module("callable_dependency_overload_provider")
CALLABLE_DEPENDENCY_OVERLOAD_CONSUMER_MODULE = wp.Module("callable_dependency_overload_consumer")
...
@wp.func(module=CALLABLE_DEPENDENCY_OVERLOAD_PROVIDER_MODULE)
def callable_dependency_overload_target(x: float):
    return x + 1.0


@wp.func(module=CALLABLE_DEPENDENCY_OVERLOAD_CONSUMER_MODULE)
def callable_dependency_apply_overload(x: int):
    return float(x)


@wp.func(module=CALLABLE_DEPENDENCY_OVERLOAD_CONSUMER_MODULE)
def callable_dependency_apply_overload(g: TypingCallable, x: float):
    return g(x)


@wp.kernel(module=CALLABLE_DEPENDENCY_OVERLOAD_CONSUMER_MODULE)
def callable_dependency_overload_kernel(out: wp.array[float]):
    out[0] = callable_dependency_apply_overload(callable_dependency_overload_target, 2.0)
...
class TestFuncCallable(unittest.TestCase):
    ...
    def test_callable_builtin_argument_target_affects_module_hash(self):
        """Verify changing a global built-in Callable target changes the module hash."""
        global CALLABLE_BUILTIN_TARGET

        original_target = CALLABLE_BUILTIN_TARGET
        try:
            CALLABLE_BUILTIN_TARGET = wp.sin
            sin_hash = callable_builtin_global_target_kernel.module.hash_module()

            CALLABLE_BUILTIN_TARGET = wp.cos
            cos_hash = callable_builtin_global_target_kernel.module.hash_module()
        finally:
            CALLABLE_BUILTIN_TARGET = original_target

        self.assertNotEqual(sin_hash, cos_hash)

    def test_callable_builtin_default_target_affects_module_hash(self):
        """Verify changing a default built-in Callable target changes the module hash."""
        original_defaults = callable_apply_builtin_default.defaults.copy()
        try:
            callable_apply_builtin_default.defaults["g"] = wp.sin
            sin_hash = callable_builtin_default_target_kernel.module.hash_module()

            callable_apply_builtin_default.defaults["g"] = wp.cos
            cos_hash = callable_builtin_default_target_kernel.module.hash_module()
        finally:
            callable_apply_builtin_default.defaults = original_defaults

        self.assertNotEqual(sin_hash, cos_hash)

    def test_callable_overload_target_affects_module_hash(self):
        """Verify changing a non-primary Callable overload target changes the module hash."""
        global CALLABLE_OVERLOAD_TARGET

        original_target = CALLABLE_OVERLOAD_TARGET
        try:
            CALLABLE_OVERLOAD_TARGET = callable_double_it
            double_hash = callable_overloaded_global_target_kernel.module.hash_module()

            CALLABLE_OVERLOAD_TARGET = callable_triple_it
            triple_hash = callable_overloaded_global_target_kernel.module.hash_module()
        finally:
            CALLABLE_OVERLOAD_TARGET = original_target

        self.assertNotEqual(double_hash, triple_hash)

    def test_callable_overload_default_target_affects_module_hash(self):
        """Verify changing a non-primary Callable overload default changes the module hash."""
        callable_overload = callable_apply_overloaded_default.get_overload([float], {})
        original_defaults = callable_overload.defaults.copy()
        try:
            callable_overload.defaults["g"] = callable_double_it
            double_hash = callable_overloaded_default_target_kernel.module.hash_module()

            callable_overload.defaults["g"] = callable_triple_it
            triple_hash = callable_overloaded_default_target_kernel.module.hash_module()
        finally:
            callable_overload.defaults = original_defaults
    ...
    def test_callable_argument_target_updates_module_dependents(self):
        ...
        cases = (
            ...
            (
                "overload",
                CALLABLE_DEPENDENCY_OVERLOAD_PROVIDER_MODULE,
                CALLABLE_DEPENDENCY_OVERLOAD_CONSUMER_MODULE,
                callable_dependency_overload_kernel,
            ),
        )
