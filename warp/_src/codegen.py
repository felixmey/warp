...
def is_regular_builtin_callable_target(func):
    """Return whether ``func`` is a simple built-in ``Callable`` target.

    Callable parameters can specialize on built-ins that lower directly through
    ...
    Returns:
        ``True`` if every overload of ``func`` is supported as a Callable target.
    """
...
def get_callable_arg_values(func, bound_args):
    ...
    Returns:
        A mapping from Callable parameter names to concrete Warp functions, or
        ``None`` when the call has no concrete Callable targets.
    """
    ...
    for name, value in bound_args.items():
        ...
        if value.is_builtin() and not is_regular_builtin_callable_target(value):
            raise WarpCodegenError(
                "Callable parameters support user-defined Warp functions and simple built-in Warp functions "
...
def get_default_arg_value(func, name, value):
    ...
    Callable defaults are specialization inputs, not runtime constants, so they
    ...
    Returns:
        A Warp function for Callable defaults, otherwise a constant ``Var``.
    """
...
def resolve_callable_arg_target(adj, arg_node, callable_arg_values=None):
    ...
    Args:
        ...
        arg_node: AST node or default value bound to a Callable parameter.
        callable_arg_values: Specialized Callable targets already bound in the
...
def resolve_call_func_overload(adj, func, call_node, callable_arg_values=None):
    """Resolve ``func`` to the overload selected by static call arguments.

    Callable target discovery needs the same overload that normal call
    resolution will choose. If any argument cannot be resolved statically, this
    returns the original function so callers can fall back to conservative
    behavior.

    Args:
        adj: Adjoint whose symbols and globals should be used for resolution.
        func: Function object referenced by the call.
        call_node: AST call node whose arguments select the overload.
        callable_arg_values: Specialized Callable targets already bound in the
            caller, keyed by parameter name.

    Returns:
        The selected overload when it can be resolved, otherwise ``func``.
    """
    ...
    overload = func.get_overload(tuple(arg_types), kwarg_types)
    return overload or func
...
def iter_call_callable_arg_targets(adj, func, call_node, callable_arg_values=None):
    ...
    Args:
        adj: Adjoint whose symbols and globals should be used for resolution.
        func: Function object referenced by the call.
        call_node: AST call node whose arguments may include Callable targets.
        callable_arg_values: Specialized Callable targets already bound in the
            caller, keyed by parameter name.

    Yields:
        Concrete Warp functions supplied to Callable parameters by explicit
    ...
    func = resolve_call_func_overload(adj, func, call_node, callable_arg_values)
...
def specialize_callable_func(func, callable_arg_values):
    ...
    Callable targets affect generated code but are omitted from the native C++
    ...
    types while storing the concrete Callable targets on the new adjoint.

    Args:
        func: User-defined function to specialize.
        callable_arg_values: Mapping from Callable parameter names to concrete
            Warp functions.
